...
loncrossing = np.asarray([-262.6, -245.4, -227.6, -208.9, -197.6, -179.6, -166.9, -138.5, -115.3, -90.25, -62.75, -56.85, np.nan, np.nan, 
                          -38.05, -14.65, 6.75, 26.95, 43.45, 61.25, 80])
latcrossing = np.asarray([np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, -66.28, -61.37, -62.45, -68.99, 
                          np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan])
distancecrossing = np.asarray([260, 514, 773, 1027, 1309, 1611, 1940, 2303, 2628, 2918, 
                               3213,  3469,  3696,  3940, 4253, 4586, 4888, 5176, 5465, 5736, 6001], dtype='int32')
...
# reverse distances so they increment to the west. 
distance_along_contour_r = np.copy(distance_along_contour)
for d in range(len(distance_along_contour_r)):
    distance_along_contour_r[d] = distance_along_contour[-(d+1)]#.data

# create distance array
lats_arr = xr.DataArray(lat_along_contour, coords={'distance': distance_along_contour_r}, dims=['distance'])
lons_arr = xr.DataArray(lon_along_contour, coords={'distance': distance_along_contour_r}, dims=['distance'])
...
## Find particle counts, transport and percentages that make it to adjacent shelf region
## AND which also travel 400km downstream (~10 degrees longitude at average latitude of shelf break isobath)
...
distidx = np.where(distance_arr.distance >= distance_arr.distance[distancecrossing[i]] + 400)[0][-1]
# find the longitude at this distance downstream
# THIS WON'T WORK FOR PENINSULA - use separate python script to calculate connectivity at Peninsula.
distlon = distance_arr.lon[distidx].values
distlat = distance_arr.lat[distidx].values
distidx2 = np.where(distance_arr.distance >= distance_arr.distance[distancecrossing[i]] + 800)[0][-1]
distlon2 = distance_arr.lon[distidx2].values
distlat2 = distance_arr.lat[distidx2].values

# find indices where particles cross from basin in east, to adjacent basin in west
idx = np.unique(np.where(ds.basindiff == shelf_east_diffs[i])[1])
print(len(idx))
...
# To speed up loop below, first find the particles (indices) that also cross to the next basin downstream 
# (on the shelf). These have travelled the required 400km, and this reduces the size of the loop.
idxtmp = np.unique(np.where(dstmp.basindiff == shelf_east_diffs[i])[1])
#if i == 0:
...
for j, k in enumerate(idxtmp):
    if j % 50000 == 0:
        print(j,k)
    crossid = np.where(dstmp.basindiff[:,k] == shelf_east_diffs[i])[0]
    if len(crossid) > 1:
    ...
    if i == 13:
        timeindxs = np.where((dslatlontmp.lat[crossid:,k] >= distlat) & (dslatlontmp.lat[crossid:,k] <= distlat2) )[0]
    else:
...
if i == 20:
    transids = np.unique(np.where(ds.basin_ZonalConn == basin_values[0])[1])
else:
...
connectivity_count.conn_count[i,0] = len(arr)
connectivity_count.conn_count_pct[i,0] = len(arr)/len(transids)*100
connectivity_count.conn_trans[i,0] = np.abs(dsivtmp.trans[arr]).sum()
connectivity_count.conn_trans_pct[i,0] = connectivity_count.conn_trans[i,0] / np.abs(ds_iv.trans[transids]).sum()*100
